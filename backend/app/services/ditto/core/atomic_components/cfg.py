...
def parse_cfg(cfg_pkl, data_root, replace_cfg=None):
    ...
    if isinstance(replace_cfg, dict):
        # Extract device if provided to propagate it
        global_device = replace_cfg.get("device")
        
        for k, v in replace_cfg.items():
            if not isinstance(v, dict):
                # If it's a top-level setting like 'device', propagate it to sub-configs
                if k == "device":
                    for sub_k in cfg.get("base_cfg", {}):
                        if isinstance(cfg["base_cfg"][sub_k], dict):
                            cfg["base_cfg"][sub_k]["device"] = v
                    if "audio2motion_cfg" in cfg:
                        cfg["audio2motion_cfg"]["device"] = v
                continue
            for kk, vv in v.items():
                if k in cfg:
                    cfg[k][kk] = vv
                    
        # Ensure global device is set if not already done
        if global_device:
            if "audio2motion_cfg" in cfg:
                cfg["audio2motion_cfg"]["device"] = global_device
            for sub_k in cfg.get("base_cfg", {}):
                if isinstance(cfg["base_cfg"][sub_k], dict):
                    cfg["base_cfg"][sub_k]["device"] = global_device
    # ---
